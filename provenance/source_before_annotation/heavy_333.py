# heavy_333.py
# Section 3.3.3: sensitivity of the frozen Gaussian RBPB selector to
# heavy-tailed sampling. The true transport map stays exactly known
# (same t_nu location-scatter family on both sides), the exact affine
# population loss stays valid (it only uses the source mean and
# covariance), and the estimator's Gaussian sampling law is what breaks.
# Frozen design (333-G1..G7):
#   geometry: the 3.3.2 G4-N pair (Sigma1 = D4, Sigma2 = Q D4 Q');
#   cells: {t5, t3} x {(48,192), (216,24)}; matching Gaussian cells are
#     reused from 3.3.2, not rerun;
#   data: X = sqrt((nu-2)/W)[:,None] * (Z @ C.T), one chi-square radial
#     per observation, Z drawn before W in each stream (frozen order),
#     so Cov(X) = Sigma exactly; C is the Cholesky factor of Sigma;
#   policies: m = 0, the frozen RBPB selector untouched (grid, B = 250,
#     tie and sentinel rules), and the dense-reference best common
#     multiplier on {0, 1/8, ..., 32};
#   reference rule: t5 minimises the mean loss curve; t3 minimises
#     the per-replicate median loss ratio because t3 has no finite
#     fourth moment, so ordinary variance-based Monte Carlo standard
#     errors are not relied upon; both curves are stored;
#   R_ref = 5000, R_confirm = 10000, no extension gate; robust
#     summaries are primary for t3;
#   failures recorded as NA, never repaired (as in 3.3.2).
...
# frozen roots and index orders (stage_index: 0 = reference, 1 = confirm;
# the inner key carries it even though only the confirmatory stage runs
# the selector, so key uniqueness never depends on that invariant)
GEOM_ROOT = 20260825
...
def t_sample(n, nu, C, rng):
    # location-scatter t_nu with covariance exactly C C'; Z before W,
    # one radial per observation (frozen order)
...
def exact_loss(m1h, m2h, A, Sigma1, A0_true):
    # exact population affine loss (population means are zero)
    v = m2h - A @ m1h
    D = A - A0_true
...
def main():
    ...
    for di, dname, nu in DISTRIBUTIONS:
        for ai, rho, n1, n2 in ALLOCATIONS:
            ...
            # reference stage: dense true-loss curves, failures as NA
            ...
            # frozen rule: t5 selects on the mean curve, t3 on the
            # median loss-ratio curve
            ...
            # confirmatory stage: fixed 10000 fresh datasets
            t0 = time.perf_counter()
            rows, failures = [], 0
            ...
            if nu > 4:
                # t5 has finite fourth moments, so mean summaries and the
                # paired ratio-of-means Monte Carlo SE remain appropriate.
