# rbpb_selector.py
# RBPB selector for the raw full-covariance Gaussian plug-in map.
# Every simulation script and the real-data application imports
# select_epsilon from this file; the criterion is implemented once, here.
#
# Frozen settings (recorded in Methods 2.5): theory scale tau, the 18-point
# multiplier grid with sentinel 24, B = 250 for simulations and B = 4000
# for real-data calls, common random numbers across candidates, smallest
# multiplier on ties, raw covariances with no clipping.
#
# Run  python rbpb_selector.py 
...
SENTINEL = 24.0
B_SIM = 250       # simulations
B_APP = 4000      # real-data calls
CHUNK = 250       # draws per block, keeps d=10 arrays small


def make_rng(seed):
    # seed is a tuple of integers, e.g. (20260901, cell, rep, stream)
    return np.random.default_rng(np.random.SeedSequence(list(seed)))


def sqrt_and_inv(M, name="matrix"):
    # symmetric square root and inverse square root; refuse non-PD input
...
def sample_wishart(d, nu, B, rng):
    # Bartlett construction of W_d(I, nu)
    if nu < d:
...
def sample_cov(Sigma, nu, B, rng):
    # bootstrap sample covariances: nu * Sigma_hat ~ W_d(Sigma, nu)
    R, _ = sqrt_and_inv(Sigma, "fitted covariance")
    return (R @ sample_wishart(Sigma.shape[0], nu, B, rng) @ R) / nu


def prep_eig(S1, S2):
    # the eigendecomposition shared by all epsilon candidates
    sq, isq = sqrt_and_inv(S1, "source covariance")
    C = sq @ S2 @ sq
    C = 0.5 * (C + np.swapaxes(C, -1, -2))
    w, V = np.linalg.eigh(C)
    # round-off can leave tiny negative eigenvalues in this product; set
    # those to zero, but treat anything materially negative as an error
...
def A_from(prep, eps):
    # entropic coefficient A_eps; rationalised form of sqrt(w + c^2) - c
    # so that tiny eigenvalues do not suffer cancellation
    isq, w, V = prep
    c = eps / 4.0
...
def criterion_matrix(S1, S2, A0, n1, n2, nu1, nu2, eps_list, B, rng):
    # Rao-Blackwellised losses, one row per candidate, common draws
    d = S1.shape[0]
    if nu1 <= d + 3:
...
def fit_moments(X, Y):
    # sample means and raw covariances, estimated-means regime
...
def pick_smallest(q, multipliers):
    # smallest multiplier among numerical ties on the criterion scale
...
def select_epsilon(X, Y, purpose, seed):
    ...
    # seed: tuple of integers, unique per call, e.g. (ROOT, cell, rep, stream)
    ...
    # sentinel diagnostics: a 24 pick is the coverage warning, and a
    # criterion still falling from 18 to 24 is the earlier warning
    tail = L[-1] - L[-2]
...
def apply_map(res, X):
    # the fitted affine map b(x) = m2 + A (x - m1)
    X = np.atleast_2d(np.asarray(X, float))
    return res["m2"] + (X - res["m1"]) @ res["A"].T


# checks (the completion standard) 

def chi2_nodes(nu, n):
    # Gauss-Legendre on (0,1) mapped to (0,inf) with the chi2 density
...
def run_checks():
    ...
    # Riccati equation and the d=1 closed form
    rng = make_rng((1,))
    ...
    # Rao-Blackwell formula: on one explicit set of draws, the criterion
    # equals the mean of the full loss over simulated bootstrap means
    rng = make_rng((2,))
    ...
    # fixed-seed reproducibility and scale equivariance
    ...
    # sample-size gates and the no-clipping rule
    try:
    ...
    # d=1: the criterion agrees with deterministic quadrature
    rng = make_rng((9,))
    ...
    # end-to-end runs in d=1 and d=5, both purposes
