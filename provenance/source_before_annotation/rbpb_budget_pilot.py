# rbpb_budget_pilot.py
# Budget pilot for the RBPB selector (internal calibration, not Results).
# Candidate budgets are prefixes of one shared 8000-draw selection stream
# on the frozen 18-point production grid (imported from rbpb_selector, so
# the grid lives in exactly one place). Each candidate pick is compared
# with the B_ref = 8000 pick on an independent evaluation stream, so the
# same noise never both selects and grades. Outer datasets are shared with
# the grid pilot (same data root and cell indices). P4 is included because
# the zero-stability criterion needs a cell that actually selects zero.
...
# (cell index, label, d, n1, n2, Sigma1, Sigma2); indices match the grid
# pilot so the outer datasets are identical
CELLS = [
...
def main():
    ...
    for ci, label, d, n1, n2, S1t, S2t in CELLS:
        ...
        for rep in range(REPS):
            ...
            # one independent evaluation of all distinct picks
        ...
        # timing: one fresh call per budget, rep-0 data of this cell
