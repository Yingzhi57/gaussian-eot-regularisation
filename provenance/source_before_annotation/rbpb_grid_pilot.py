# rbpb_grid_pilot.py
# Grid pilot for the RBPB selector (internal design audit, not Results).
# Checks that the base grid M0 is wide and fine enough using a pilot-only
# reference grid: 1/16 and 1/8 below, midpoints of adjacent positive base
# points inside, and 18, 20, 22, 24, 32 above (18/20/22 are the documented
# amendment after the first pilot exposed the gap between 16 and 24).
# One stream selects on both grids; when the two picks differ, a second
# independent stream re-evaluates just {0, base pick, reference pick}, so
# the same noise never both selects a point and grades it.
# Primitives and their checks live in rbpb_selector.py.
...
def geom_cov(d, kappa, rot=None):
    # geometric spectrum with condition number kappa, trace normalised to d
...
def main():
    ...
    for ci, (label, d, n1, n2, S1t, S2t) in enumerate(CELLS):
        ...
        for rep in range(REPS):
            ...
            # is the criterion still falling from 24 to 32?
            tail = L[I32] - L[I24]
            falling = tail.mean() < -2 * tail.std(ddof=1) / np.sqrt(B_SELECT)

            # independent re-evaluation of the two picks when they differ
            reg = se = 0.0
            if jb != jr:
                cols = [0.0]
