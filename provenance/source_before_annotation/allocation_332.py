# allocation_332.py
# Section 3.3.2: when regularisation helps in full covariance, across
# source-target allocation, conditioning and non-commutativity.
# Implements the frozen 332-G1..G5 design:
#   geometries (index): 0 = G4-C commuting control (Sigma1 = D4,
#     Sigma2 = reversed D4), 1 = G4-N moderate noncommuting (the 3.3.1
#     pair), 2 = G4-K conditioning stress (kappa = 16, same rotation Q);
#   allocations (index 0..6): n1 + n2 = 240 with n1/n2 in
#     {1/4, 1, 3, 7, 9, 11, 15};
#   per cell, a dense-reference stage (R_ref datasets, mesh {0,...,32}
#     step 1/8) fixes the best common multiplier, then an independent
#     confirmatory stage compares m = 0, the known-basis reference
#     m_KB(rho) = ((11 - rho)/(1 + rho))_+, the fixed dense-reference
#     multiplier, and the RBPB selection (frozen grid, B = 250);
#   losses are exact population affine losses, so Monte Carlo variation
#     comes only from training samples and the inner bootstrap;
#   confirmatory repetitions extend from 5000 to 10000 in a cell only if
#     the normalized paired RBPB-vs-zero MCSE exceeds 0.002 (appending
#     replications, never rerunning);
#   failures (non-SPD, gate failures) are recorded as NA, never repaired,
#     and any failure at d = 5 with n2 >= 15 is itself a diagnostic.
# Population matrices are built once, and the realised Q and all
# (Sigma1, Sigma2, A0) go into the outputs, not just the seed.
# No figure here: the main visual belongs to G7.
...
# frozen roots (G5.8): geometry seed unchanged, fresh 3.3.2 namespaces
GEOM_ROOT = 20260825
...
def D_matrix(kappa):
    # geometric spectrum, trace normalised to 5, condition number kappa
...
def m_kb(rho):
    # known-basis leading reference under estimated means
...
def main():
    ...
    for gi, (gname, S1_true, S2_true) in enumerate(GEOMETRIES):
        ...
        for ai, (rho, n1, n2) in enumerate(ALLOCATIONS):
            ...
            # confirmatory stage on fresh independent datasets (compute)
            t0 = time.perf_counter()
            mkb = m_kb(rho)
            rows, failures = [], 0
