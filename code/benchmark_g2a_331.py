# Small-sample balanced Gaussian benchmark for the full-matrix RBPB rule.
# This case uses d=5 and n1=n2=50. Independent reference datasets estimate the
# best common multiplier on a dense mesh; fresh datasets evaluate it against
# no regularisation, the local known-basis comparator and RBPB. Geometry, data
# and bootstrap draws use separate random-number streams.
# Run: python benchmark_g2a_331.py
...
# Case 2 uses a seed namespace distinct from Case 1.
CELLS = [
    (1, "G2a d=5 (50,50) small-sample check", 5, 50, 50,
...
def main():
    ...
    emit("3.3.1 Gaussian benchmark (G2a variant: d = 5, n1 = n2 = 50 (small-sample check); protocol per frozen G3)")
    ...
    for ci, label, d, n1, n2, S1_true, S2_true in CELLS:
        ...
        np.savez("benchmark_331_g2a_reference.npz",
        ...
        if ci == 1:
            ...
            fig.savefig("benchmark_331_g2a.pdf", bbox_inches="tight")
            fig.savefig("benchmark_331_g2a.png", dpi=150, bbox_inches="tight")
            emit("   figure written: benchmark_331_g2a.pdf / .png")
    ...
    with open("benchmark_331_g2a_report.txt", "w") as f:
        f.write("\n".join(lines) + "\n")
    with open("benchmark_331_g2a_records.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(all_rows[0].keys()))
        w.writeheader()
        w.writerows(all_rows)
    print("\nwritten: benchmark_331_g2a_report.txt, benchmark_331_g2a_records.csv, "
          "benchmark_331_g2a_reference.npz")
