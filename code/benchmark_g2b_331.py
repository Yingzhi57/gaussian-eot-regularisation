# Ten-dimensional balanced Gaussian benchmark for the full-matrix RBPB rule.
# This case uses d=10 and n1=n2=100. Independent reference datasets estimate
# the best common multiplier on a dense mesh; fresh datasets evaluate it
# against no regularisation, the local known-basis comparator and RBPB.
# Geometry, data and bootstrap draws use separate random-number streams.
# Run: python benchmark_g2b_331.py
...
# Fixed benchmark design and independent random-number roots.
...
# Case 3 uses a seed namespace distinct from Cases 1 and 2.
CELLS = [
    (2, "G2b d=10 (100,100) dimension check", 10, 100, 100,
     geom_cov(10, 4.0), geom_cov(10, 4.0, rotation(10))),
]
...
def main():
    ...
    emit("3.3.1 Gaussian benchmark (G2b variant: d = 10, n1 = n2 = 100 (dimension check); protocol per frozen G3)")
    ...
    for ci, label, d, n1, n2, S1_true, S2_true in CELLS:
        ...
        np.savez("benchmark_331_g2b_reference.npz",
        ...
        # Evaluate the four decision rules on fresh datasets.
        t0 = time.perf_counter()
        rows = []
        failures = 0
        rep = 0
        ...
        # Save the benchmark diagnostic figure.
        if ci == 2:
            ...
            fig.savefig("benchmark_331_g2b.pdf", bbox_inches="tight")
            fig.savefig("benchmark_331_g2b.png", dpi=150, bbox_inches="tight")
            emit("   figure written: benchmark_331_g2b.pdf / .png")
    ...
    with open("benchmark_331_g2b_report.txt", "w") as f:
        f.write("\n".join(lines) + "\n")
    with open("benchmark_331_g2b_records.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(all_rows[0].keys()))
        w.writeheader()
        w.writerows(all_rows)
    print("\nwritten: benchmark_331_g2b_report.txt, benchmark_331_g2b_records.csv, "
          "benchmark_331_g2b_reference.npz")
